...
def sequence_to_filters(protein: th.Tensor,
                        ...
                        norm :bool = True):
    r'''
    create 3d filters with shape of:
    (num_filters, 1, (kernel_size1, kernel_size2), emb_size)
    ...
    seqlen1, seqlen2, emb_size = protein.shape
    assert seqlen1 == seqlen2
    filter_list = []
    # from (N, N, E) array
    # slices are in form of (1, N, E)
    #filt = protein.narrow(0, 0, kernel_size)
    #filter_list.append(filt)
    for start in range(0, seqlen1, filter_stride):
            #single filter of size (1, kerenl_size, num_emb_feats)
            filt = protein.narrow(0, start, kernel_size)
            filter_list.append(filt)
    #filt = protein.narrow(0, start, kernel_size)
    #filter_list.append(filt)
    filters = th.cat(filter_list, 0).unsqueeze(1)

    #normalize filters
    if norm:
        norm_val = filters.view(seqlen1, -1).pow(2).sum(1, keepdim=True)
        norm_val[norm_val == 0] = 1e-5
        norm_val = norm_val.sqrt().view(seqlen1, 1, 1, 1)
        filters /= norm_val
    return filters.to(device)

def calc_density(protein: th.Tensor, filters: th.Tensor) -> th.FloatTensor:
    '''
