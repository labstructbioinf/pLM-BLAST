...
for pos, (idx, row) in enumerate(df.iterrows()):
	q_str = f'{row.qstart+1}-{row.qend+1}'
	t_str = f'{row.tstart+1}-{row.tend+1}'
...
for pos, (idx, row) in enumerate(df.iterrows()):
	...
	print(f'Q {row.qstart+1:>6} {row.qseq} {row.qend+1:<6}')
	print(f'         {row.con}')
	print(f'T {row.tstart+1:>6} {row.tseq} {row.tend+1:<6}')
